...
4. **Naming**: display name `"{panel_path} - {label}"` via `descriptor_display_name`; suggested object id `slugify(f"{devid}_{symbol}")` (stable catalog key — not `module_name` / i18n labels); `_attr_has_entity_name = True`; DeviceInfo identifiers `(domain, devid)` in **flat** grouping (default), manufacturer `"BragerOne"`. Optional **group by menu** (`CONF_DEVICE_GROUPING`) places entities with a stable bootstrap `menu_key` (router `name` / `path`, never localized titles) on child devices `identifiers={(domain, f"{devid}:{menu_key}")}` with `via_device=(domain, devid)`; entities without `menu_key` stay on the parent. Connection entities use SPA `module` i18n labels (never hardcoded). Changing `suggested_object_id` only affects **new** entity allocations; existing entity_ids are not rewritten by HA. Opting into menu grouping remaps device membership (document for users).
