...
4. **Naming**: display name `"{panel_path} - {label}"` via `descriptor_display_name`; suggested object id `slugify(f"{devid}_{symbol}")` (stable catalog key — not `module_name` / i18n labels); `_attr_has_entity_name = True`; DeviceInfo identifiers `(domain, devid)`, manufacturer `"BragerOne"`. Connection entities use SPA `module` i18n labels (never hardcoded). Changing `suggested_object_id` only affects **new** entity allocations; existing entity_ids are not rewritten by HA.
