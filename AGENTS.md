...
1. **Push, not poll**: state-bearing entities set `_attr_should_poll = False` and update via `runtime.add_listener()` (and `add_connectivity_listener()` for module online/offline, `add_cloud_session_listener()` / `add_live_push_listener()` for library↔cloud session and zombie push health — both also drive parameter-entity `available`). Never add polling or a coordinator. Command-only entities (`button.py`) have no ParamStore state but still listen for connectivity/transport to drive availability.
