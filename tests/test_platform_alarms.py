...
@pytest.mark.asyncio
async def test_async_refresh_alarms_marks_unavailable_on_http_error() -> None:
    """Non-success REST status must not look like a successful empty feed."""
    runtime, api = _runtime_with_alarms()

    async def unauthorized(*_a: Any, **_k: Any) -> tuple[int, Any]:
        return 401, {"alarms": []}

    async def history_ok(*_a: Any, **_k: Any) -> tuple[int, Any]:
        return 200, {"alarms": []}

    api.modules_alarms = unauthorized  # type: ignore[method-assign]
