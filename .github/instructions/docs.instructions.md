...
3. **Version references** (minimum HA version, `py-bragerone` pin, Python version) must match `hacs.json`, `manifest.json`, and `pyproject.toml`; flag any drift in the PR so it gets fixed at the source, not papered over in docs.
